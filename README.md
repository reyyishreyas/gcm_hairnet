...
## Reference Risk Map Generation

The reference risk maps used as training targets are generated by an independent lightweight Tiny CNN pipeline, provided in [`reference_map_generator/`](reference_map_generator/).

### What it does

1. Collects Sentinel-2 satellite imagery and 18 GIS features for any urban area
2. Extracts 18 GIS features at 1 km resolution
3. Builds four interpretable risk components: Human Exposure (HE), Infrastructure Complexity (IC), Urban Activity (UA), Environmental Buffer (EB)
4. Trains a Tiny CNN (`4 → 32 → 16 → 1`) to learn the mapping from risk components to continuous risk maps
5. Generates 256×256 reference risk maps for each city

### Quick start

```bash
cd reference_map_generator
pip install -r requirements.txt
python run_pipeline.py
```

Outputs are written to `data/processed/` (images, gis, labels, metadata, splits.json).

See [`reference_map_generator/README.md`](reference_map_generator/README.md) for the full pipeline, data sources, and QGIS integration.
